...
class TrainingArguments:
    ...
    use_lora: bool = field(
        default=False,
        metadata={"help": "Enable LoRA for efficient fine-tuning on both LLM and ViT."}
    )

    # --- LLM 专用 LoRA 配置 ---
    llm_lora_r: int = field(
        default=16,
        metadata={"help": "LoRA rank for the language model."}
    )
    llm_lora_alpha: int = field(
        default=16,
        metadata={"help": "LoRA alpha for the language model."}
    )
    llm_lora_dropout: float = field(
        default=0.05,
        metadata={"help": "LoRA dropout for the language model."}
    )
    llm_lora_target_modules: str = field(
        default="q_proj,v_proj,k_proj,o_proj",
        metadata={"help": "Comma-separated target modules for LLM (e.g., 'q_proj,v_proj,k_proj,o_proj')."}
    )

    # --- ViT 专用 LoRA 配置 ---
    vit_lora_r: int = field(
        default=8,
        metadata={"help": "LoRA rank for the Vision Transformer."}
    )
    vit_lora_alpha: int = field(
        default=16,
        metadata={"help": "LoRA alpha for the Vision Transformer."}
    )
    vit_lora_dropout: float = field(
        default=0.05,
        metadata={"help": "LoRA dropout for the Vision Transformer."}
    )
    vit_lora_target_modules: str = field(
        default="q_proj,v_proj",
        metadata={"help": "Comma-separated target modules for ViT (e.g., 'q_proj,v_proj')."}
    )
...
def main():
    ...
    if training_args.use_lora and training_args.visual_und:
        def parse_target_modules(module_str: str) -> list:
            return [m.strip() for m in module_str.split(',') if m.strip()]
        
        # A. LLM 部分 (Qwen2)
        llm_targets = parse_target_modules(training_args.llm_lora_target_modules)
        ...
        if dist.get_rank() == 0:
            logger.info("LoRA applied to language model with config: "
                        f"r={training_args.llm_lora_r}, alpha={training_args.llm_lora_alpha}, "
                        f"targets={llm_targets}")
            model.language_model.print_trainable_parameters()

        # 3.2 ViT的LoRA（如果启用）
        if training_args.visual_und:
            vit_lora_config = LoraConfig(
                r=training_args.vit_lora_r,
                lora_alpha=training_args.vit_lora_alpha,
                target_modules=parse_target_modules(training_args.vit_lora_target_modules),
                lora_dropout=training_args.vit_lora_dropout,
                bias="none",
            )
            model.vit_model = get_peft_model(model.vit_model, vit_lora_config)

        # 强制关闭外部冻结（让LoRA接管）
        training_args.freeze_llm = False
        training_args.freeze_vit = False

        if dist.get_rank() == 0:
            logger.info("LoRA injected successfully AFTER loading pretrained weights.")
            model.language_model.print_trainable_parameters()
            if training_args.visual_und:
    ...
    logger.info("Creating PackedDataset...")
    ...
    logger.info("PackedDataset created.")
